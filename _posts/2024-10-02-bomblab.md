...
title: Bomblab
description: A hands-on journey through the Binary Bomb Lab from CMU, where we defuse a binary 'bomb' through reverse engineering and assembly code analysis. Perfect for learning x86 assembly, GDB debugging, and binary analysis.
categories: [blogs]
---

本文为 CSAPP bomblab （THU ICS版本）的解题报告。题目以及源文件详见[这里]({{ site.url }}/files/bomblab.zip)。以下为解题过程。
